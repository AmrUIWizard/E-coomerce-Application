...
- [7 Query Performance Optimization](#7-query-performance-optimization)
	- [7.1 Total Products per Category](#71-total-products-per-category)
	- [7.2 Top Customers by Total Spending](#72-top-customers-by-total-spending)
	- [7.3 Latest 1000 Orders by Date](#73-latest-1000-orders-by-date)
	- [7.4 Products That Have Stock Less Than 5](#74-products-that-have-stock-less-than-5)
	- [7.5 Total Revenue for Each Category](#75-total-revenue-for-each-category)
-  [8 Denormalization Notes](#8-denormalization-notes)
	- [8.1 Add customer_name into `orders` table](#81-add-customer_name-into-orders-table)
...
### 7.1 Total Products per category

| Original Query                                                                                                                                                                                    | Execution Time (Before)                                                                                                                                              | Optimization Technique                                                                                                                                                              | Rewritten Query                                                                                                                                                                                                                                                     | Execution Time (After)                                                                                                                                                           |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SELECT C.CATEGORY_ID, C.CATEGORY_NAME, COUNT(P.PRODUCT_ID) FROM CATEGORY C LEFT JOIN PRODUCT P ON C.CATEGORY_ID = P.CATEGORY_ID GROUP BY C.CATEGORY_ID, C.CATEGORY_NAME ORDER BY C.CATEGORY_ID;` | **102.976 ms**  <br>• Seq Scan: 15.7 ms (100k rows)  <br>• Hash Right Join: 32.3 ms  <br>• HashAggregate: 16.2 ms  <br>• Sort: 0.1 ms  <br>• Rows: 100,000 processed | **• Query:** Correlated subquery pattern  <br>**• Index:** Created covering index for index-only scans  <br>**• Impact:** Eliminated sequential scan, enabled index-only processing | `CREATE INDEX idx_product_category_covering ON product(category_id, product_id);`  <br>  <br>`SELECT c.category_id, c.category_name, (SELECT COUNT(*) FROM product p WHERE p.category_id = c.category_id) as product_count FROM category c ORDER BY c.category_id;` | **54.430 ms**  <br>• Index Scan (category): 0.5 ms  <br>• SubPlan loops: 100 times  <br>• Index Only Scan: 0.4 ms per loop  <br>• Heap Fetches: 0  <br>• Memory: Minimal per-row |

### 7.2 Top Customers by Total Spending

| Original Query                                                                                                                                                        | Execution Time (Before)                                                                                                                            | Optimization Technique                                                                                                                                                                               | Rewritten Query                                                                                                                           | Execution Time (After)                                                                                                                                   |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SELECT o.customer_id, o.customer_name, SUM(o.total_amount) AS total_spent FROM orders o GROUP BY o.customer_id, o.customer_name ORDER BY total_spent DESC LIMIT 10;` | **2913 ms**  <br>• Seq Scan: 355 ms (2.5M rows)  <br>• HashAggregate: 1246 ms  <br>• Disk Spill: 160 MB  <br>• Memory: 8249 kB  <br>• Batches: 161 | **• Index:** Created covering index `idx_orders_customer_covering`  <br>**• Impact:** Eliminated disk spills, enabled index-only scans  <br>**• Technique:** Same query structure with optimal index | `CREATE INDEX idx_orders_customer_covering ON orders(customer_id, customer_name) INCLUDE (total_amount);`  <br>  <br>Same query structure | **1430 ms**  <br>• Index Only Scan: 555 ms  <br>• GroupAggregate: 718 ms  <br>• No Disk Spill: 0 MB  <br>• Memory: In-memory only  <br>• Heap Fetches: 0 |

### 7.3 Latest 1000 Orders by Date

| Original Query                                              | Execution Time (Before)                                                                                                                                                                                      | Optimization Technique                                                                                   | Rewritten Query                                                                            | Execution Time (After)                                                                                                                                          |
| ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SELECT * FROM orders ORDER BY order_date DESC LIMIT 1000;` | **706 ms**<br>• Parallel Seq Scan: ~421 ms per worker (7.5M rows)<br>• Gather Merge: ~18 ms<br>• Top-N Heapsort (per worker): ~653 ms<br>• Buffers: shared hit=8,725 read=145,991<br>• Rows processed: ~7.5M | • Index aligned with `ORDER BY`<br>• Backward B-tree index scan<br>• Eliminated full table scan and sort | `CREATE INDEX idx_orders_date_covering ON orders(order_date);`<br><br>Same query structure | **16 ms**<br>• Index Scan Backward: 16.0 ms<br>• Index searches: 1<br>• Rows read: 1,000 only<br>• Buffers: shared hit=32 read=945<br>• No sort, no parallelism |

### 7.4 Products That Have Stock Less Than 5

| Original Query                                                                     | Execution Time (Before)                                                                                                                          | Optimization Technique                                                                                                 | Rewritten Query                                                                                                                                             | Execution Time (After)                                                                                                                          |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `SELECT * FROM product WHERE stock_quantity <= 5;` (no limit, table ~100,000 rows) | **15.648 ms**<br>• Seq Scan: full table (100,000 rows)<br>• Filter: stock_quantity <= 5<br>• Buffers: shared hit=1137<br>• Rows matching: 49,980 | • Created index on `stock_quantity`<br>• Used Bitmap Index Scan + Heap Scan<br>• Optimizes fetching only matching rows | `CREATE INDEX idx_products_quantity_covering ON product(stock_quantity);`<br>`SELECT * FROM product WHERE stock_quantity <= 5;`<br><br>Same query structure | **11.288 ms**<br>• Bitmap Index Scan → Bitmap Heap Scan<br>• Index Searches: 1<br>• Buffers: shared hit=1137 read=44<br>• Rows matching: 49,980 |

### 7.5 Total Revenue for Each Category

| Original Query                                                                                                                                                                                                                                                                        | Execution Time (Before)                                                                                                                                                                                                                           | Optimization Technique                                                                                                                                             | Rewritten Query                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Execution Time (After)                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SELECT c.category_id, c.category_name, SUM(od.quantity * od.unit_price) AS total_revenue FROM category c JOIN product p ON c.category_id = p.category_id JOIN order_details od ON p.product_id = od.product_id GROUP BY c.category_id, c.category_name ORDER BY total_revenue DESC;` | **3055 ms**<br>• Parallel Seq Scan on `order_details`: ~7.5M rows<br>• Parallel Hash Join (`order_details → product`)<br>• Late aggregation at category level<br>• Disk reads: ~47k buffers<br>• Execution dominated by hash join and aggregation | • Early aggregation (product-level pre-aggregation)<br>• Covering index enables index-only scan<br>• Reduced join cardinality<br>• Merge Join instead of Hash Join | `CREATE INDEX idx_order_details_product_covering ON order_details(product_id) INCLUDE (quantity, unit_price);`<br><br>`SELECT c.category_id, c.category_name, SUM(odr.total_revenue) AS total_revenue FROM category c JOIN product p ON c.category_id = p.category_id JOIN ( SELECT product_id, SUM(quantity * unit_price) AS total_revenue FROM order_details GROUP BY product_id ) odr ON p.product_id = odr.product_id GROUP BY c.category_id, c.category_name ORDER BY total_revenue DESC; ` | **2015 ms**<br>• Parallel Index-Only Scan on `order_details`<br>• Heap Fetches: 0<br>• Rows reduced to ~100k before joins<br>• Merge Join used<br>• Disk reads reduced to ~29k buffers |
